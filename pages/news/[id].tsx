  ...
  return (
    <>
      ...
      <main className='mx-auto max-w-[850px] gap-4 py-20 px-5 sm:px-10'>
        ...
        <h2 className='hd1 pt-5 pb-7'>{article.title}</h2>
...
export async function getServerSideProps({
  ...
  const [article, news] = await Promise.all([
    ...
    apiFetch<PaginatedApiResponse<News>>('news?language=dv'),
  ]);
  ...
  return article
    ? article.language === 'en'
      ? {
          redirect: {
            destination: `/en/news/${article.id}`,
          },
        }
      : {
          props: {
            ...article,
            news: news?.results.filter((m) => m.id !== article?.id),
          },
        }
    : { notFound: true };
}
