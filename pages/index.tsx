        ...
        {homepage?.banners?.map((banner, i) => (
          ...
          >
            <div className='flex-1'>
              ...
              <span className='tag-primary'>އިތުރު މައުލޫމާތު</span>
            </div>
            <img
...
function MorePage({ page }: { page: number }) {
  const { data } = useSWRImmutable<NewsList>(`news?language=dv&page=${page}`);
  return data?.results?.map((item) => <HomeItem key={item.id} {...item} />);
}

export async function getServerSideProps({ res }: GetServerSidePropsContext) {
  const [news, homepage] = await Promise.all([
    apiFetch<NewsList>('news?language=dv'),
    apiFetch<HomePage>('fixed-pages?page=HomePage&language=dv'),
  ]);
